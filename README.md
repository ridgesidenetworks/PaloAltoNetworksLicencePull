# PANW Licence Key Retrieval

This repository contains a small Python script to download `.key` licence files for a specific Palo Alto Networks NGFW serial number using the Palo Alto Networks licensing API.
...
For each licence returned by the API, the script writes a `.key` file in the form:
